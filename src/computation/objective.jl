...
function compute_objective(model::DefiMarkovitzModel)::Float64
    ν, δ, λ, γ = compute_nu_delta_lambda_gamma(model; verbose = false)
    if δ <= 0 || γ <= 0 || λ == 0
        return NaN
    end
    ψ, φ = compute_psi_phi(model; verbose = false)
    norm_x, x1, norm_y, y1, norm_z, z1, x_dot_y, x_plus_y_dot_z = compute_x_y_z(model, ν, γ, ψ; verbose = false)
    c1, c2, c3 = compute_c(model, ψ, φ)
    Λ0, Λ1, Λ2, Λ3, Λ4, Λ5, Λ6 = compute_all_lambdas(
        norm_x,
        norm_y,
